...
\begin{boxdefinition}[Formulae]
    A set of \textbf{formulae} is a non-empty set of strings in $A$.
\end{boxdefinition}

For the remainder of this subsection, fix a set $\mathcal{F}$ of formulae. It will be important to distinguish the formulae that will serve as our most basic assumptions from those that will be derived from them.
...
\begin{boxdefinition}[Recursive Formal Deduction Systems]
    Suppose there exists an algorithm that can test whether a string is a formula or an axiom. Then, $\Sigma$ is called a \textbf{recursive formal deduction system}.
\end{boxdefinition}
